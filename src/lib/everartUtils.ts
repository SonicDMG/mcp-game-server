...
function initializeEverartSdk(): boolean {
    ...
    if (!everartApiKey) {
        logger.warn('[EverArt Utils] EVERART_API_KEY environment variable not set. Image generation disabled.');
        return false;
    }
    try {
        everart = new EverArt(everartApiKey);
        logger.info('[EverArt Utils] EverArt client initialized.');
        return true;
    } catch (sdkError) {
        logger.error('[EverArt Utils] Failed to initialize EverArt SDK:', sdkError);
...
export async function generateImageWithPolling(prompt: string, modelId: string = EVERART_MODEL_ID): Promise<string | undefined> {
    if (!initializeEverartSdk() || !everart) {
        logger.warn('[EverArt Utils] SDK not initialized, cannot generate image.');
        return undefined;
    }
    ...
    logger.info(`[EverArt Utils] Attempting to generate image with prompt: "${prompt}"`);
    try {
        ...
        logger.debug('[EverArt Utils] Raw response from EverArt create:', JSON.stringify(generationResponse, null, 2));
        ...
        if (Array.isArray(generationResponse) && generationResponse.length > 0 && generationResponse[0].id) {
            ...
            logger.error('[EverArt Utils] Unexpected response structure from EverArt create:', generationResponse);
            generationId = undefined;
        }

        // Poll for result if we got an ID
        if (typeof generationId === 'string') {
            logger.info(`[EverArt Utils] EverArt generation started with ID: ${generationId}. Polling for result...`);
            const startTime = Date.now();
            while (Date.now() - startTime < MAX_POLL_DURATION_MS) {
                await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));
                const statusCheck = await everart.v1.generations.fetch(generationId);
                logger.debug(`[EverArt Utils] Polling status for ${generationId}: ${statusCheck.status}`);

                if (statusCheck.status === 'SUCCEEDED') {
                    imageUrl = statusCheck.image_url ?? undefined;
                    logger.info(`[EverArt Utils] EverArt generation ${generationId} succeeded. Image URL: ${imageUrl}`);
                    break; // Exit loop
                } else if (statusCheck.status === 'FAILED' || statusCheck.status === 'CANCELED') {
                    logger.error(`[EverArt Utils] EverArt generation ${generationId} failed or was canceled. Status: ${statusCheck.status}`);
                    break; // Exit loop
                }
            ...
            if (!imageUrl && Date.now() - startTime >= MAX_POLL_DURATION_MS) {
                logger.warn(`[EverArt Utils] EverArt generation ${generationId} timed out after ${MAX_POLL_DURATION_MS / 1000} seconds.`);
            }
        } else {
            logger.error('[EverArt Utils] Failed to get a valid generation ID from EverArt create call.');
        }

    } catch (everartError) {
        logger.error('[EverArt Utils] Error calling EverArt API:', everartError);
